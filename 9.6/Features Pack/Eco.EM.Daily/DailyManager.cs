...
namespace Eco.EM.Daily
{
    public class DailyManager : IModKitPlugin, IInitializablePlugin
    {
        ...
        public DailyManager()
        {
            ...
            if (!File.Exists(Defaults.SaveLocation + _subPath + _packsFile))
                SavePacks();
        }
        ...
        {
            return FileManager<DailyConfig>.ReadTypeHandledFromFile(Defaults.SaveLocation + _subPath, _configFile);
        }

        private DailyData LoadData()
        {
            return FileManager<DailyData>.ReadTypeHandledFromFile(Defaults.SaveLocation + _subPath, _dataFile);
        }

        private RewardPackData LoadPacks()
        {
            return FileManager<RewardPackData>.ReadTypeHandledFromFile(Defaults.SaveLocation + _subPath, _packsFile);
        }

        public static void SaveConfig()
        {
            FileManager<DailyConfig>.WriteTypeHandledToFile(Config, Defaults.SaveLocation + _subPath, _configFile);
        }

        public static void SaveData()
        {
            FileManager<DailyData>.WriteTypeHandledToFile(Data, Defaults.SaveLocation + _subPath, _dataFile);
        }

        public static void SavePacks()
        {
            FileManager<RewardPackData>.WriteTypeHandledToFile(PackData, Defaults.SaveLocation + _subPath, _packsFile);
        }
