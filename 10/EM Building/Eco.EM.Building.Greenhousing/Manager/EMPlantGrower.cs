...
namespace Eco.EM.Building.Greenhousing
{
    public class EMPlantGrower : PlantGrower
    {
        ...
        {
            // Prevents NAN from getting into Yield... should yield grow if the plant isn't growing?
            if (layerValues[0] == 0f)
                return;
            ...
            var remainingTotalGrowth = layerValues[0] * area.Area * GrowthRateModifier * 3;
            for (var juvenileIndex = 0; juvenileIndex < juvenilePlants.Count; juvenileIndex++)
            {
                ...
                //Calculate time until matures based on last tick growth. (i.e. tick of 30mins it growed 0.1. so in order to reach 1 it will need 10 iterations, making TimeTillMatureHours = 5 hours)
                juvenilePlant.YieldPercent += GreenhousingExtensions.VaildGreenhouse(juvenilePlant.Position.XYZi(), out _) ? actualGrowth * 1.5f: layerValues[1] * actualGrowth;
