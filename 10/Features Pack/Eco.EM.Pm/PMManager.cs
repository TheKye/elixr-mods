namespace Eco.EM.Pm
{
    ...
    public class PMManager : IModKitPlugin
    {
        ...
        public PMManager()
        {
            Defaults.CreateDirectoryIfNotExist(Defaults.SaveLocation + _subPath);
            if (!File.Exists(Path.Combine(Defaults.SaveLocation + _subPath, _logFile)))
        ...
        {
            StreamReader stream = File.OpenText(Path.Combine(Defaults.SaveLocation + _subPath, _logFile));
