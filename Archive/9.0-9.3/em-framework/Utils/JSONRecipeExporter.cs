...
namespace ECO.EM.Tools
{
    ...
    public class JSONRecipeExporter : IModKitPlugin, ICommandablePlugin
    {
        ...
        List<Recipe> VanillaRecipes = new List<Recipe>();
        Dictionary<string, RecipeData> ExportData = new Dictionary<string, RecipeData>();
        ...
        {
            FileManager<Dictionary<string,RecipeData>>.WriteTypeHandledToFile(ExportData,Base.SaveLocation + _subPath, _exportFile);
        }
        ...
        {
            if (!Directory.Exists(Path.Combine(Base.SaveLocation + _subPath)))
                Directory.CreateDirectory(Path.Combine(Base.SaveLocation + _subPath));

            if (!File.Exists(Path.Combine(Base.SaveLocation + _subPath, _exportFile)))
                File.Create(Path.Combine(Base.SaveLocation + _subPath, _exportFile));
        }
