...
namespace Eco.EM.ModkitTools
{
    ...
    public class PassiveCraftingComponent : WorldObjectComponent
    {
        ...
        private List<(Item, float)> Outputs = new List<(Item, float)>();

        // this component is only Enabled if all its conditions are met
        public override bool Enabled => CheckConditions();
        private List<IPassiveCraftCondition> Conditions = new List<IPassiveCraftCondition>();
        ...
        {
            List<(Item, float)> prods = new List<(Item, float)>();
            foreach (var e in recipe.Items) { prods.Add((e.Item, e.Quantity.GetBaseValue)); };

            List<(Item, float)> ings = new List<(Item, float)>();
        ...
        private void Tick(float deltaTime)
        {
            ...
            {
                status.SetStatusMessage(false, "This station is unowned and unable to passively produce");
                return;
            }
            ...
            status.SetStatusMessage(result, "Passively Crafting", failString == "" ? "Unable to craft while inventory is full" : failString);
        }
        ...
        protected virtual Result Craft() 
        {
            ...
            using var changes = new InventoryChangeSet(inv);
