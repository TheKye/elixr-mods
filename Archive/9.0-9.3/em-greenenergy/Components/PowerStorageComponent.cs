...
namespace Eco.EM.GreenEnergy
{
    ...
    public class PowerStorageComponent : WorldObjectComponent, IPersistentData
    {
        ...
        {
            if (GridInterface == null) { this.status.Message = string.Format(Localizer.DoStr("No Grid locally found, please relocate to a power grid. Current charge unknown")); return; }
        ...
        {
            var disabledMessage = string.Format(Localizer.DoStr("Battery is not attached to grid. Current charge: {0}%"), percentCharge);
            ...
            if (Charging)
                enabledMessage = string.Format(Localizer.DoStr("Battery is in Charge Mode. Current charge {0}%."), percentCharge);
            else
                enabledMessage = string.Format(Localizer.DoStr("Battery is in Discharge Mode. Current charge: {0}%."), percentCharge);
