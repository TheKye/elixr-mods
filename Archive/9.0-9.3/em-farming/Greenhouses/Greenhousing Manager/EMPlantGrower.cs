...
namespace Eco.EM.Greenhousing
{
    public class EMPlantGrower : PlantGrower
    {
        ...
        public override void Apply(WorldArea area, float[] layerValues, WorldLayerNeighborInfo[] neighborValues)
        {
            // Prevents NAN from getting into Yield
            if (layerValues[0] == 0f) return;

            var juvenilePlants = new List<Plant>();
            foreach (var plant in EcoSim.PlantSim.PlantsInArea(area))
            {
                if (plant.Species == species && plant.GrowthPercent < 1f && !plant.GrowthBlocked && plant.Alive)
                    juvenilePlants.Add(plant);
            }
            ...
            var remainingTotalGrowth = layerValues[0] * area.Area;
