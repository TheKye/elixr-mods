...
namespace EM.ECO.MOTD
{
    public class MOTDManager : IInitializablePlugin, IModKitPlugin
    {
        ...
        public MOTDManager()
        {
            ...
            if (!File.Exists(Path.Combine(Base.SaveLocation + _subPath, _dataFile)))
                SaveData();

            if (!File.Exists(Path.Combine(Base.SaveLocation + _subPath, _configFile)))
                SaveConfig();
        ...
        {
            return FileManager<MOTDConfig>.ReadTypeHandledFromFile(Base.SaveLocation + _subPath, _configFile);
        }

        private MOTDData LoadData()
        {
            return FileManager<MOTDData>.ReadTypeHandledFromFile(Base.SaveLocation + _subPath, _dataFile);
        }

        public static void SaveConfig()
        {
            FileManager<MOTDConfig>.WriteTypeHandledToFile(Config, Base.SaveLocation + _subPath, _configFile);
        }

        public static void SaveData()
        {
            FileManager<MOTDData>.WriteTypeHandledToFile(Data, Base.SaveLocation + _subPath, _dataFile);
        }
        ...
        {
            var path = Path.Combine(Base.SaveLocation + _subPath, _motdFile);
            char _ignore = '#';
