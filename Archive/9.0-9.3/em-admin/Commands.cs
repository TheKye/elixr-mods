...
namespace Eco.EM.Admin
{
    public class Admin : IChatCommandHandler
    {
        ...
        {
            message = $"{message}";
            ChatBaseExtended.CBAnnouncement(title, message, "EMAnnouncement");
        }
        ...
        public static void UnclaimAllProperty(User user, bool returnClaims, string targetUser)
        {
            ...
            string returnClaimText = returnClaims ? "You Chose To Refund the Land Claim Papers" : "You chose not to return Land Claims (Default)";
            int count = 0;

            if (tgtUser != null)
            {
                var prop = PropertyManager.AllOwnedDeeds(tgtUser);
                if (PropertyManager.PropertyForAlias(tgtUser).Count() >= 1)
                {
                    ...
                    ChatBaseExtended.CBInfoBox($"{tgtUser}'s Property has been unclaimed, Removed {count} Claim Plots, And {returnClaimText}", user);
                    return;
                }
                ChatBaseExtended.CBError($"No Property to unclaim.", user);
                return;
            }
            ChatBaseExtended.CBError($"Could not find anyone with the username {targetUser}.", user);
            return;
        }
        ...
        public static void ReclaimDevTool(User user, bool all = false)
        {
            ...
            ChatBaseExtended.CBInfoBox($"All Found Dev tools have been reclaimed. Reclaimed {count} Dev Tools.", user);
        }
