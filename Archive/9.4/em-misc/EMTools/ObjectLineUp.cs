...
namespace Eco.EM.Framework.Testing
{
    public class ObjectLineUp : IChatCommandHandler
    {
        ...
        public static void NamespaceLineUp(User user, string itemlookup, bool _namespace = true)
        {
            ...
            else user.Player.MsgLoc($"No World Objects found in the {(_namespace ? "Namespace" : "Assembly")}", MessageCategory.Error);

            if (itemList.Count != 0) FillChests(user, itemList);
            else user.Player.MsgLoc($"No Items found in the {(_namespace ? "Namespace" : "Assembly")}", MessageCategory.Error);
        }
