...
namespace Eco.EM.Framework.Permissions
{
    ...
    public class EMCustomChatProcessor : ICommandProcessorHandler
    {
        ...
        public static bool EMProcessCommand(ChatCommand command, IChatClient chatClient)
        {
            ...
            if (adapter == null)
            {
                chatClient.ErrorLocStr(string.Format(Base.appName + Localizer.DoStr("Command {0} not found"), command.Name));
                return false;
            }
            ...
            chatClient.ErrorLocStr(string.Format(Base.appName + Localizer.DoStr("You are not authorized to use the command {0}"), command.Name));
