...
namespace Eco.EM.Homes
{
    public class HomeCommands : IChatCommandHandler
    {
        ...
        public static void ConfigureHomes(User user, string setting, int value)
        {
            ...
            {
                case "max":
                    ChatBaseExtended.CBInfoBox(Base.appName + string.Format(Localizer.DoStr("<color=yellow>Home Max Teleports</color> has been changed to <color=green>{0}</color>"),value), user);
                    break;
                case "cost":
                    ChatBaseExtended.CBInfoBox(Base.appName + string.Format(Localizer.DoStr("<color=yellow>Home Teleport Calorie Cost</color> has been changed to <color=green>{0}</color>"),value), user);
                    break;
                case "count":
                    ChatBaseExtended.CBInfoBox(Base.appName + string.Format(Localizer.DoStr("<color=yellow>Max Home Count</color> has been changed to <color=green>{0}</color>"),value), user);
                    break;
                case "cooldown":
                    ChatBaseExtended.CBInfoBox(Base.appName + string.Format(Localizer.DoStr("<color=yellow>Home Cooldown Seconds</color> has been changed to <color=green>{0}</color>"),value), user);
                    break;
                default:
                    ChatBaseExtended.CBInfoBox(Base.appName + string.Format(Localizer.DoStr("Homes {0} has been set to {1}"),setting ,value), user);
                    break;
            }
        ...
        public static void GroupConfigureHomes(User user, string groupName, string setting, int value)
        {
            ...
            {
                ChatBaseExtended.CBError(Base.appName + string.Format(Localizer.DoStr("Groups cannot have a smaller maximum value than the serverwide maximum {0}. Reduce the serverwide maximum first."), HomeManager.Config.MaxTeleports), user);
                return;
            }
            ...
            {
                case "max":
                    ChatBaseExtended.CBInfoBox(Base.appName + string.Format(Localizer.DoStr("<color=yellow>Home Max Teleports</color> for {0} has been changed to <color=green>{1}</color>"), group.GroupName, value), user);
                    break;
                case "cost":
                    ChatBaseExtended.CBInfoBox(Base.appName + string.Format(Localizer.DoStr("<color=yellow>Home Teleport Calorie Cost</color> for {0} has been changed to <color=green>{1}</color>"), group.GroupName, value), user);
                    break;
                case "count":
                    ChatBaseExtended.CBInfoBox(Base.appName + string.Format(Localizer.DoStr("<color=yellow>Max Home Count</color> for {0} has been changed to <color=green>{1}</color>"), group.GroupName, value), user);
                    break;
                case "cooldown":
                    ChatBaseExtended.CBInfoBox(Base.appName + string.Format(Localizer.DoStr("<color=yellow>Home Cooldown Seconds</color> for {0} has been changed to <color=green>{1}</color>"), group.GroupName, value), user);
                    break;
                default:
                    ChatBaseExtended.CBInfoBox(Base.appName + string.Format(Localizer.DoStr("Homes {0} for {1} has been set to {2}"), setting, group.GroupName, value), user);
                    break;
            }
        ...
        public static void GoHome(User user, string homeName)
        {
            ...
            {
                ChatBaseExtended.CBError(Base.appName + Localizer.DoStr("Please specify a Home name."), user);
                return;
            }
            ...
            {
                ChatBaseExtended.CBError(Base.appName + string.Format(Localizer.DoStr("You have exhausted all your Home Teleports today. ({0} out of {1})"), HomeManager.Data.GetMaxTeleports(user) - userData.Teleports, HomeManager.Data.GetMaxTeleports(user)), user);
                return;
            }

            if (user.Stomach.Calories < HomeManager.Data.GetMinCalorieCost(user))
            {
                ChatBaseExtended.CBError(Base.appName + Localizer.DoStr("You're too hungry to teleport."), user);
                return;
            }

            if (userData.OnCoolDown)
            {
                ChatBaseExtended.CBError(Base.appName + string.Format(Localizer.DoStr("Your ability to teleport is on cool down. It will reset in {0} seconds."), Math.Ceiling(HomeManager.Data.GetMinCoolDownTime(user) - check.Subtract(userData.CoolDownSetTime).TotalSeconds)), user);
                return;
            }
            ...
            if (home == null)
            {
                ChatBaseExtended.CBError(Base.appName + string.Format(Localizer.DoStr("Your home: {0} was not found."), homeName), user);
                return;
            }

            if (home.Location.WorldPosition3i == user.Position.WorldPosition3i)
            {
                ChatBaseExtended.CBInfoBox(Base.appName + Localizer.DoStr("You arrived before you left. No calories burnt."), user);
                return;
            }
            ...
            var total = HomeManager.Data.GetMaxTeleports(user) - userData.Teleports;
            if (HomeManager.Data.GetMaxTeleports(user) == int.MaxValue)
                ChatBaseExtended.CBInfoBox(Base.appName + string.Format(Localizer.DoStr("You teleported to {0}. You have an unlimited amount of Home Teleports left today"), home.HomeName), user);
            else
                ChatBaseExtended.CBInfoBox(Base.appName + string.Format(Localizer.DoStr("You teleported to {0}. You have {1} Home Teleports left today"), home.HomeName, total), user);
        ...
        public static void HomeInfo(User user)
        {
            ...
            ChatBaseExtended.CBInfoPane($"{user.Name}'s Homes", Base.appName + text, "EMHomes", user);
        }
        ...
        {
            var plot = PropertyManager.GetPlotFromWorldPos(user.Player.Position.XZi);
            if (plot == null || !plot.IsAuthorized(user).Success)
            {
                ChatBaseExtended.CBError(Base.appName + Localizer.DoStr("You cannot set home on land you have no rights on."), user);
                return;
            }
            ...
            {
                ChatBaseExtended.CBError(Base.appName + string.Format(Localizer.DoStr("You've already set your maximum allowed number of homes ({0})."), userMax), user);
                return;
            }

            if (!userData.AddHome(homeName, user.Position))
            {
                ChatBaseExtended.CBError(Base.appName + Localizer.DoStr("You already have a home with that name."), user);
                return;
            }

            ChatBaseExtended.CBInfoBox(Base.appName + string.Format(Localizer.DoStr("You added <color=green>{0}</color> to your homes. You have set {1} of {2} allowed homes."), homeName, userData.Homes.Count, userMax), user);
            HomeManager.SaveData();
        }
        ...
        public static void RemoveHome(User user, string homeName)
        {
            ...
            {
                ChatBaseExtended.CBError(Base.appName + string.Format(Localizer.DoStr("Unable to find home {0}"), homeName), user);
                return;
            }

            ChatBaseExtended.CBInfo(Base.appName + string.Format(Localizer.DoStr("You removed {0} from your homes. You now have {1} homes."), homeName, userData.Homes.Count), user);
            HomeManager.SaveData();
        }
        ...
        public static void ClearHomes(User user)
        {
            ...
            userData.ClearHomes();
            ChatBaseExtended.CBInfoBox(Base.appName + string.Format(Localizer.DoStr("You have removed all of your homes. You now have {0} homes."), userData.Homes.Count), user);
            HomeManager.SaveData();
        }
        ...
        public static void homeReload(User user)
        {
            ...
            ChatBaseExtended.CBInfoBox(Base.appName + Localizer.DoStr("Home Config Reloaded, use /home-settings to see if the reload worked."), user);
        }
            ...
            if(group != null)
            {
                ...
                group.ForEach(grp =>
                {
                    ...
                    if (homePermission != null)
                    {
                        ...
                        _message += Localizer.DoStr("Max Home Teleports: ") + config.MaxHomeCount + "\n";
                        _message += Localizer.DoStr("Max Home Count: ") + config.MaxTeleports + "\n";
