...
namespace Eco.EM.Daily
{
    public class DailyCommands : IChatCommandHandler
    {
        ...
        public static void ConfigureDaily(User user, string setting, int value)
        {
            ...
            {
                case "xp":
                    ChatBaseExtended.CBInfoBox(Base.appName + string.Format(Localizer.DoStr("<color=yellow>Daily Reward Experience</color> has been changed to <color=green>{0}</color>."), value), user);
                    break;
                case "starttier":
                    ChatBaseExtended.CBInfoBox(Base.appName + string.Format(Localizer.DoStr("<color=yellow>Daily Start Tier</color> has been changed to <color=green>{0}</color>."), value), user);
                    break;
                case "time":
                    ChatBaseExtended.CBInfoBox(Base.appName + string.Format(Localizer.DoStr("<color=yellow>Daily Minimum Logged-in Time</color> has been changed to <color=green>{0}</color>."), value), user);
                    break;
                case "timer":
                    ChatBaseExtended.CBInfoBox(Base.appName + string.Format(Localizer.DoStr("<color=yellow>Daily Claim Timer</color> has been changed to <color=green>{0}</color>."), value), user);
                    break;
                default:
                    ChatBaseExtended.CBInfoBox(Base.appName + string.Format(Localizer.DoStr("Daily Config {0} has been set to {1}."), setting, value), user);
                    break;
            }
        ...
        public static void GroupConfigureDaily(User user, string groupName, string setting, int value)
        {
            ...
            {
                case "xp":
                    ChatBaseExtended.CBInfoBox(Base.appName + string.Format(Localizer.DoStr("<color=yellow>Daily Reward Experience</color> for {0} has been changed to <color=green>{1}</color>."),group.GroupName ,value), user);
                    break;
                case "starttier":
                    ChatBaseExtended.CBInfoBox(Base.appName + string.Format(Localizer.DoStr("<color=yellow>Daily Start Tier</color> for {0} has been changed to <color=green>{1}</color>."), group.GroupName, value), user);
                    break;
                case "time":
                    ChatBaseExtended.CBInfoBox(Base.appName + string.Format(Localizer.DoStr("<color=yellow>Daily Minimum Logged-in Time</color> for {0} has been changed to <color=green>{1}</color>."), group.GroupName, value), user);
                    break;
                case "timer":
                    ChatBaseExtended.CBInfoBox(Base.appName + string.Format(Localizer.DoStr("<color=yellow>Daily Claim Timer</color> has been changed to <color=green>{0}</color>."), value), user);
                    break;
                default:
                    ChatBaseExtended.CBInfoBox(Base.appName + string.Format(Localizer.DoStr("Daily Config {0} for {1} has been set to {2}."), group.GroupName, value), user);
                    break;
            }
