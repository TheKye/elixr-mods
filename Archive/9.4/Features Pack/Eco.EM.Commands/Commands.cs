namespace Eco.EM.Commands
{
    ...
    class Commands : IChatCommandHandler
    {
        ...
        public static void Stats(User user, string target)
        {
            ...
            string gender = string.Format(Localizer.DoStr("Character Gender: {0}"), targetUser.Avatar.Sex);
            gender = gender[0].ToString().ToUpper() + gender.Remove(0, 1);

            string skilltitle = string.Format(Localizer.DoStr("Skill Title: {0}"), targetUser.Skillset.Title);
            ...
            PlayerStats += string.Format(Localizer.DoStr("Food Skill Rate: {0}"), (targetUser.UserXP.SkillRate - targetUser.HomePropertyValue.TotalSkillPoints)) + "\n";
            PlayerStats += string.Format(Localizer.DoStr("House Skill Rate: {0}"), (targetUser.HomePropertyValue.TotalSkillPoints)) + "\n";
            PlayerStats += string.Format(Localizer.DoStr("Total Skill Rate: {0}"), targetUser.UserXP.SkillRate) + "\n";
            PlayerStats += string.Format(Localizer.DoStr("SP: {0}"), String.Format("{0:0}", targetUser.UserXP.XP)) + "\n";

            ChatBaseExtended.CBInfoPane(Title, PlayerStats, "PlayerStats", user);
        }
        ...
        {
            ChatBaseExtended.CBInfoPane(Localizer.DoStr("Admins"), ListToString(Base.Admins), "PlayerStats", user);
        }

        [ChatCommand("Displays a list of admins that are currently online.", "admins-online")]
        public static void AdminsOnline(User user)
        {
            ChatBaseExtended.CBInfoPane(Localizer.DoStr("Admins Online"), ListToString(Base.OnlineAdmins), "PlayerStats", user);
        }
