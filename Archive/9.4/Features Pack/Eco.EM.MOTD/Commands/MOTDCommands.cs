...
namespace Eco.EM.MOTD
{
    public class MOTDCommands: IChatCommandHandler
    {
        ...
        public static void AddMOTDMessage(User user, string msg)
        {
            ...
            ChatBaseExtended.CBChat(Base.appName + string.Format(Localizer.DoStr("Added to MOTD: {0}"),msg), user, ChatBase.MessageType.Temporary);
        ...
        public static void RemMOTDMessage(User user, int index)
        {
            ...
            {
                ChatBaseExtended.CBChat(Base.appName + string.Format(Localizer.DoStr("Message {0} could not be found to remove."), index), user, ChatBase.MessageType.Temporary);
                return;
            }

            ChatBaseExtended.CBChat(Base.appName + string.Format(Localizer.DoStr("Removed: {0}"), MOTDManager.Data.Messages[index - 1]), user, ChatBase.MessageType.Temporary);
