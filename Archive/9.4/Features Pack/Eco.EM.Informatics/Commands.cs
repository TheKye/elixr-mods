namespace Eco.EM.Informatics
{
    ...
    public class Informatics : IModKitPlugin, IChatCommandHandler
    {
        ...
        public static void Prices(User user, string ItemType)
        {
            ...
            if (tag == null)
                item = CommandsUtil.ClosestMatchingEntity(user.Player, itemType, Item.AllItems, x => x.GetType().Name, x => x.DisplayName);
            ...
            if (buyOffers.Count > 0)
            {
                if (tag != null)
                {
                    ...
                    {
                        message += $"Buying ({o.tagItemName}) {o.Quantity} for {o.Price} {o.Currency} \t <color=green>{o.StoreName}</color>\t{o.StoreOwner}\n";
                    });
                }
                else
                {
