namespace Eco.EM.Warp
{
    ...
    public class WarpCommands : IChatCommandHandler
    {
        ...
        public static void ConfigureWarp(User user, string setting, int value)
        {
            ...
            {
                case "max":
                    ChatBaseExtended.CBInfoBox(Base.appName + string.Format(Localizer.DoStr("<color=yellow>Warps Max Teleports</color> has been changed to <color=green>{0}</color>"),value), user);
                    break;
                case "cost":
                    ChatBaseExtended.CBInfoBox(Base.appName + string.Format(Localizer.DoStr("<color=yellow>Warp Teleport Calorie Cost</color> has been changed to <color=green>{0}</color>"),value), user);
                    break;
                case "cooldown":
                    ChatBaseExtended.CBInfoBox(Base.appName + string.Format(Localizer.DoStr("<color=yellow>Warp Cooldown Seconds</color> has been changed to <color=green>{0}</color>"),value), user);
                    break;
                default:
                    ChatBaseExtended.CBInfoBox(Base.appName + string.Format(Localizer.DoStr("Warps {0} has been changed to {1}"),setting, value), user);
                    break;
            }
        ...
        public static void GroupConfigureWarp(User user, string groupName, string setting, int value)
        {
            ...
            {
                ChatBaseExtended.CBError(Base.appName + string.Format(Localizer.DoStr("Groups cannot have a smaller maximum value than the serverwide maximum {0}. Reduce the serverwide maximum first."), WarpManager.Config.MaxTeleports), user);
                return;
            }
            ...
            {
                case "max":
                    ChatBaseExtended.CBInfoBox(Base.appName + string.Format(Localizer.DoStr("<color=yellow>Warps Max Teleports</color> for {0} has been changed to <color=green>{1}</color>"), group.GroupName, value), user);
                    break;
                case "cost":
                    ChatBaseExtended.CBInfoBox(Base.appName + string.Format(Localizer.DoStr("<color=yellow>Warp Teleport Calorie Cost</color> for {0} has been changed to <color=green>{1}</color>"), group.GroupName, value), user);
                    break;
                case "cooldown":
                    ChatBaseExtended.CBInfoBox(Base.appName + string.Format(Localizer.DoStr("<color=yellow>Warp Cooldown Seconds</color> for {0} has been changed to <color=green>{1}</color>"), group.GroupName, value), user);
                    break;
                default:
                    ChatBaseExtended.CBInfoBox(Base.appName + string.Format(Localizer.DoStr("Warps {0} for {1} has been changed to {2}"),setting, group.GroupName, value), user);
                    break;
            }
        ...
        public static void Warpto(User user, string pointName)
        {
            ...
            {
                ChatBaseExtended.CBError(Base.appName + Localizer.DoStr("You must specify where you want to warp to."), user);
                return;
            }
            ...
            {
                ChatBaseExtended.CBError(Base.appName + string.Format(Localizer.DoStr("You have exhausted all your teleports today. ({0} out of {1})"), WarpManager.Data.GetMaxTeleports(user) - userData.Teleports, WarpManager.Data.GetMaxTeleports(user)), user); // Add the time remaining here??
                return;
            }

            if (userData.OnCoolDown)
            {
                ChatBaseExtended.CBError(Base.appName + string.Format(Localizer.DoStr("Your ability to teleport is on cool down. It will reset in {0} seconds."),Math.Ceiling(WarpManager.Data.GetMinCoolDownTime(user) - check.Subtract(userData.CoolDownSetTime).TotalSeconds)), user);
                return;
            }
            ...
            if (warp == null)
            {
                ChatBaseExtended.CBError(Base.appName + string.Format(Localizer.DoStr("{0} does not exist as a community warp location."), pointName), user);
                return;
            }
            ...
            {
                ChatBaseExtended.CBError(Base.appName + Localizer.DoStr("You are too hungry to warp."), user);
                return;
            }
        ...
        public static void warpReload(User user)
        {
            ...
            ChatBaseExtended.CBInfo(Base.appName + Localizer.DoStr("Warp Config Reloaded, use /warp-settings to see if the reload worked."), user);
        }
        ...
        public static void warpDelete(User user, string warpName)
        {
            ...
            if (point == null) { ChatBaseExtended.CBError(Base.appName + Localizer.DoStr("Warp point not found, check name and try again."), user); return; }
            ...
            if (WarpManager.Data.RemovePoint(point.PointName))
            {
                ...
                ChatBaseExtended.CBInfo(Base.appName + string.Format(Localizer.DoStr("{0} removed."), point.PointName), user);
                return;
            }

            ChatBaseExtended.CBError(Base.appName + string.Format(Localizer.DoStr("{0} was unable to be removed."), point.PointName), user);
        }
    }
}
