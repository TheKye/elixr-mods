...
namespace Eco.EM.Admin.ReportsSystem
{
    public class AdminCommands : IChatCommandHandler
    {
        ...
        public static void IssueWarning(IChatClient chatClient, User targetUser, string message, int warningTime = 30)
        {
            ...
            //Remove all leading and trailing whitespaces just incase for int parsing.
        ...
        public static void ConfigureReports(IChatClient chatClient, string setting, string value)
        {
            ...
            switch (settings)
            {
                case "enablereports":
                    switch (values.ToLower())
                    {
                        case "true":
                            ...
                            admin.SaveConfig();
                            chatClient.MsgLocStr($"Report System has been: {GreenText("Enabled")}, If you haven't be sure to set the WebhookURL");
                            return;
                        case "false":
                            admin.Config.EnableReports = false;
                            admin.SaveConfig();
                            chatClient.MsgLocStr($"Report System has been: {RedText("Disabled")}");
                            return;
                        default:
                ...
                case "enablehelp":
                    switch (values.ToLower())
                    {
                        case "true":
                            ...
                            admin.SaveConfig();
                            chatClient.MsgLocStr($"Help System has been: {GreenText("Enabled")}, If you haven't be sure to set the WebhookURL");
                            return;
                        case "false":
                            admin.Config.EnableHelpRequests = false;
                            admin.SaveConfig();
                            chatClient.MsgLocStr($"Help System has been: {RedText("Disabled")}");
                            return;
                        default:
                ...
                case "adminrole":
                    ...
                    admin.SaveConfig();
                    chatClient.MsgLocStr($"Admin Role to notify has been set to: {GreenText(values)}, Please ensure this is correct.");
                    return;
                case "notifyadmins":
                    switch (values.ToLower())
                    {
                        case "true":
                            ...
                            admin.SaveConfig();
                            chatClient.MsgLocStr($"Admin Notifications has been: {GreenText("Enabled")}, If you haven't be sure to set the WebhookURL");
                            return;
                        case "false":
                            admin.Config.NotifyAdmin = false;
                            admin.SaveConfig();
                            chatClient.MsgLocStr($"Admin Notifications has been: {RedText("Disabled")}");
                            return;
                        default:
                ...
                case "webhookurl":
                    ...
                    admin.SaveConfig();
                    chatClient.MsgLocStr($"Discord Webhook URL has been set to: {GreenText(values)}, Please ensure this is correct.");
                    return;
                case "servername":
                    admin.Config.MultiServerReportName = values;
                    admin.SaveConfig();
                    chatClient.MsgLocStr($"Multi Server Report Name has been set to: {GreenText(values)}, Please ensure this is correct.");
                    return;
                default:
